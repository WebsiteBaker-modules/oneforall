...
require(WB_PATH.'/modules/admin.php');
    ...
    $sReplaceSysvar = (function ($sContent) {
        $aReg = array (
            'AppUrl' => WB_URL.'/',
...
$sSqlSet = 'SELECT * FROM `'.TABLE_PREFIX.'mod_'.$mod_name.'_items` WHERE `item_id` = '.$item_id.' ';
// Get item
if ($query_item = $database->query($sSqlSet)) {
    $fetch_item = $query_item->fetchRow(MYSQLI_ASSOC);
}
$fetch_item['scheduling'] = (empty($fetch_item['scheduling']) ? $aScheduleDefault : $fetch_item['scheduling']);

// Get item scheduling
$scheduling = (__unserialize($fetch_item['scheduling']));
...
$setting_img_section = $database->get_one('SELECT `img_section` FROM `'.TABLE_PREFIX.'mod_'.$mod_name.'_page_settings` WHERE `section_id` = '.(int)$section_id.' ');
...
if (file_exists(WB_PATH.$datepicker_lang_path)) {
    echo '<script src="'.WB_URL.$datepicker_lang_path.'"></script>';
}

// Load jquery timepicker js
// The jquery timepicker css is loaded by @import() in the backend.css stylesheet
echo '<script src="'.WB_URL.'/modules/'.$mod_name.'/js/timepicker/js/jquery-ui-timepicker-addon.js"></script>';
...
if (file_exists(WB_PATH.$timepicker_lang_path)) {
    echo '<script src="'.WB_URL.$timepicker_lang_path.'" ></script>';
}
?>
<script >
...
<form id="modify" action="<?php echo WB_URL; ?>/modules/<?php echo $mod_name; ?>/save_item.php" method="post" enctype="multipart/form-data" style="margin: 0;">
    ...
    <table class="mod_ofa_modify_item" id="mod_<?php echo $mod_name; ?>_modify_item_b" >
        <tbody>
        <tr>
            <td><?php echo $MOD_ONEFORALL[$mod_name]['TXT_TITLE']; ?>:</td>
            <td>
                <input type="text" name="title" id="title" style="width:90%;" maxlength="150" value="<?php echo $fetch_item['title']; ?>" />
            </td>
        </tr>
        <tr>
            ...
            <td>
                ...
                // )
                $aScheduling = (is_array($scheduling) ? array_filter($scheduling): false);
                if ((($aScheduling === false) || (is_array($aScheduling) && count($aScheduling)==0))) {
                ?>
...
?>
            <select name="new_section_id" style="width: 240px">
<?php
...
<?php
    ...
    <?php if($featured) { ?>
        <tr>
            <td class="align_top"><?php echo $MOD_ONEFORALL[$mod_name]['TXT_FEATURED']; ?>:</td>
            <td>
                <div style="float:left">
                    <input type="radio" name="featured" id="featured_true" value="1" <?php if ($fetch_item['featured'] == 1) { echo " checked='checked'"; } ?> />
                    <label for="featured_true"><?php echo $TEXT['YES']; ?></label>
                    &nbsp;
                    <input type="radio" name="featured" id="featured_false" value="0" <?php if ($fetch_item['featured'] == 0) { echo " checked='checked'"; } ?> />
                    <label for="featured_false"><?php echo $TEXT['NO']; ?></label>
                </div>
                <?php //if ($fetch_item['featured'] == 1) { ?>
                <div>
                    <div style="float:left; margin-right: 5px; width: 70px;" class="label">| Position:</div>
                    <div style="width:25px; float:left; margin-top: -4px;" class="value">
                        <input type="text" name="featured_pos" id="featured_pos" value="<?php echo $fetch_item['featured_pos']; ?>" />
                    </div>
                     &nbsp;(1 bis 6)
                </div>
                <?php //} ?>
    ...
    if ($query_fields->numRows() > 0) {
        while ($field = $query_fields->fetchRow(MYSQLI_ASSOC)) {
            ...
            if (!empty($fetch_item['fields'][$field_id])) {
                ...
                $value = $database->get_one('SELECT `value` FROM `'.TABLE_PREFIX.'mod_'.$mod_name.'_item_fields` WHERE `item_id` = '.(int)$item_id.' AND `field_id` = '.(int)$field_id.' ');
            ...
            // Replace MediaURL
            $value = (!is_array($value) ? $sReplaceSysvar($value) : $value);
    ...
    <table class="mod_<?php echo $mod_name; ?>_submit_b mod_ofa_modify_item ofa_bottom" >
        </tbody>
                <tr>
                    <td style="padding-left: 12px;float: left;">
                        <input name="save_and_return" type="submit" value="<?php echo $TEXT['SAVE']; ?>" style="min-width: 8.525em;" />
                        <input name="save" type="submit" value="<?php echo $MOD_ONEFORALL[$mod_name]['TXT_SAVE_AND_BACK_TO_LISTING']; ?>" style="min-width: 8.525em; margin-left: 20px;" />
                    </td>
                    <td style="padding-right: 12px;float: right;">
                    <input type="button" value="<?php echo $TEXT['CANCEL']; ?>" onclick="window.location='<?php echo ADMIN_URL; ?>/pages/modify.php?page_id=<?php echo $page_id; ?>';" style="min-width: 8.525em; float: right;" />
                    </td>
                </tr>
        </tbody>
    </table>
...
<div style="display: <?php echo $display_img_section; ?>">
    ...
    <table class="mod_ofa_modify_item" id="mod_<?php echo $mod_name; ?>_images_b">
        <thead>
            <tr>
        ...
        $top_img = $database->get_one('SELECT MAX(`position`) AS `top_position` FROM `'.TABLE_PREFIX.'mod_'.$mod_name.'_images` WHERE `item_id` = '.(int)$item_id.' ');

        // Get image data from db
        $query_image = $database->query('SELECT * FROM `'.TABLE_PREFIX.'mod_'.$mod_name.'_images` WHERE `item_id` = '.(int)$item_id.' ORDER BY `position` ASC ');
        ...
        if ($no_image) {
            echo '<tr id="mod_'.$mod_name.'_no_image_b"><td colspan="8">'."\n";
    ...
    <table class="mod_ofa_modify_item ofa_bottom">
        <tbody>
        <tr> <!--  class="mod_<?php echo $mod_name; ?>_submit_b" -->
            <td style="padding-left: 12px;float: left;">
                <input name="save_and_return_to_images" type="submit" value="<?php echo $TEXT['SAVE']; ?>" style="min-width: 8.525em;" />
                <input name="save" type="submit" value="<?php echo $MOD_ONEFORALL[$mod_name]['TXT_SAVE_AND_BACK_TO_LISTING']; ?>" style="min-width: 8.525em; margin-left: 20px;" />
            </td>
            <td  style="padding-right: 12px;float: right;">
            <input type="button" value="<?php echo $TEXT['CANCEL']; ?>" onclick="window.location='<?php echo ADMIN_URL; ?>/pages/modify.php?page_id=<?php echo $page_id; ?>';" style="min-width: 8.525em; float: right;" />
    ...
    <div id="mod_<?php echo $mod_name; ?>_resize_settings_b">
        ...
        <span><?php echo $MOD_ONEFORALL[$mod_name]['TXT_MAX_WIDTH']; ?></span>
        <input type="text" size="5" name="maxwidth" value="<?php echo $img_resize['maxwidth']; ?>" /> px
        <span><?php echo $MOD_ONEFORALL[$mod_name]['TXT_MAX_HEIGHT']; ?></span>
        <input type="text" size="5" name="maxheight" value="<?php echo $img_resize['maxheight']; ?>" /> px
        <span><?php echo $MOD_ONEFORALL[$mod_name]['TXT_JPG_QUALITY']; ?></span>
        <input type="text" size="3" name="quality" value="<?php echo $img_resize['quality']; ?>" /> %
    </div>
    <div id="uploader">
        <p style="color: red;">Your browser doesn't have Silverlight or HTML5 support.</p>
    </div>
...
//            flash_swf_url: '<?php echo WB_URL; ?>/modules/<?php echo $mod_name; ?>/js/plupload/js/Moxie.swf',
        ...
        uploader.bind('FileUploaded', function(up, file, res) {
            ...
            if (res_obj.result === null) {
                ...
                $('#mod_<?php echo $mod_name; ?>_images_b tbody').append(
                  '<tr id="id_'+img_id+'">'
                  +'<td style="width: 94px;">'
                  +'<div class="tooltip">'
                  +'<a href="<?php echo WB_URL; ?>/media/<?php echo $mod_name; ?>/images/item<?php echo $item_id; ?>/'+filename+'" target="_blank" rel="noopener">'
                  +'<img src="<?php echo WB_URL; ?>/media/<?php echo $mod_name; ?>/thumbs/item<?php echo $item_id; ?>/'+filename+'" alt="">'
                  +'<\/a>'
                  +'<div class="arrow_box"><img src="<?php echo WB_URL; ?>/media/<?php echo $mod_name; ?>/images/item<?php echo $item_id; ?>/'+filename+'" alt="" title="'+filename+'">'
                  +'<\/div>'
                  +'<\/div>'
                  +'<\/td>'
                  +'<td style="width: 351px;">'
                  +'<a href="<?php echo WB_URL; ?>/media/<?php echo $mod_name; ?>/images/item<?php echo $item_id; ?>/'+filename+'" target="_blank" rel="noopener">'+filename
                  +'<\/a>'
                  +'<\/td>'
                  +'<td style="width: 156px;">'
                  +'<input type="text" name="images['+img_id+'][title]" style="width: 150px;" maxlength="255" value="">'
                  +'<input type="text" name="images['+img_id+'][alt]" style="width: 150px;" maxlength="255" value="">'
                  +'<\/td>'
                  +'<td style="width: 206px;">'
                  +'<textarea name="images['+img_id+'][caption]" rows="3" style="width: 200px;">'
                  +'<\/textarea>'
                  +'<\/td>'
                  +'<td><\/td>'
                  +'<td><\/td>'
                  +'<td style="width: 32px;">'
                  +'<input type="checkbox" name="images['+img_id+'][active]" value="1" checked="checked">'
                  +'<\/td>'
                  +'<td><\/td>'
                  +'<\/tr>'
                  );
            ...
            // Error
                ...
                $('#mod_<?php echo $mod_name; ?>_images_b tbody').append(
                  '<tr class="plupload_error_msg">'
                  +'<td class="ui-state-error" style="width: 94px;">'
                  +'<span class="ui-icon ui-icon-alert">'
                  +'<\/span>'
                  +'<\/td>'
                  +'<td colspan="6">'
                  +'<strong>'+res_obj.error.filename
                  +'<\/strong>'
                  +res_obj.error.message+'<\/td>'
                  +'<td class="ui-state-error">'
                  +'<span class="ui-icon ui-icon-circle-close" title="<?php echo $TEXT['CLOSE']; ?>">'
                  +'<\/span>'
                  +'<\/td>'
                  +'<\/tr>'
                  );
    ...
    <script src="<?php echo WB_URL; ?>/modules/<?php echo $mod_name; ?>/js/plupload/js/plupload.full.min.js"></script>
    <script src="<?php echo WB_URL; ?>/modules/<?php echo $mod_name; ?>/js/plupload/js/jquery.ui.plupload/jquery.ui.plupload.min.js"></script>
    <?php
    ...
    if (file_exists(WB_PATH.$plupload_lang_path)) {
        echo '<script src="'.WB_URL.$plupload_lang_path.'"></script>';
    }
    ?>
    <br /><br />
</div>
