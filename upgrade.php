...
if ($continue && isset($action) && in_array($action, ['upgrade']) ) {
    ...
    if ($old_dir != $new_dir && is_dir($new_dir)) {
        ...
        if (!rename($old_dir, $new_dir)) {
            echo '<span class="bad">'.$MESSAGE['MEDIA_CANNOT_RENAME'].'</span>';
            $continue = false; $error++;
        } else {
...
// Add field 'featured' to table 'items', if not exists
if ($continue) {

    $table_name = TABLE_PREFIX.'mod_'.$mod_name.'_items';
    $field_name = 'featured';
    $parameter  = "int(11) NOT NULL DEFAULT '0' AFTER `active`";
...
// Add field 'featured_pos' to table 'items', if not exists
if ($continue) {

    $table_name = TABLE_PREFIX.'mod_'.$mod_name.'_items';
    $field_name = 'featured_pos';
    $parameter  = "int(11) NOT NULL DEFAULT '0' AFTER `featured`";
    ...
    if ($num_items > 0) {
        while ($item = $query_items->fetchRow(MYSQLI_ASSOC)) {
            ...
            if (!empty($item_link)) {
                ...
                // check for oneforall directories else create it
                // this is because subpages folders will not created automatically
                $detail_pages = WB_PATH.PAGES_DIRECTORY.$page_link;
                if ($view_detail_pages && !is_writable($detail_pages)) { make_dir($detail_pages); }
