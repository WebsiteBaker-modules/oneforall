...
// **********************************************************************************
    ...
    $('[class^="mod_oneforall_active"]').click(function () {
        ...
        $.post(WB_URL+'/modules/' + mod_oneforall.mod_name + '/ajax/toggle_active.php',
                {
                    value: bt_value,
                    item_id: item_id,
    ...
    if (jQuery().sortable) {
        ...
        $('#mod_oneforall_items_b tbody').sortable({
            ...
            update: function () {
                ...
                $.post(WB_URL+'/modules/' + mod_oneforall.mod_name+'/ajax/move_item.php',order,function () {
                    // Set the success message used by ajaxStop(), see on top of this file
    ...
    if (jQuery().sortable) {
        ...
        $('#mod_oneforall_images_b tbody').sortable({
            ...
            update: function () {
                var order = $(this).sortable('serialize')+'&action=update_pos&mod_name='+mod_oneforall.mod_name;
                // Post ordering to the server
                $.post(WB_URL+'/modules/'+mod_oneforall.mod_name+'/ajax/move_image.php',order,function () {
    ...
    if (jQuery().sortable) {
        ...
        $('#mod_oneforall_custom_fields_b > tbody').sortable({
            ...
            update: function () {
                var order = $(this).sortable('serialize')+'&action=update_pos&mod_name='+mod_oneforall.mod_name;
                // Post ordering to the server
                $.post(WB_URL+'/modules/'+mod_oneforall.mod_name+'/ajax/move_field.php',order,function () {
                    // Set the success message used by ajaxStop(), see on top of this file
                    success_msg.text(mod_oneforall.txt_dragdrop_message);
                });
            }
