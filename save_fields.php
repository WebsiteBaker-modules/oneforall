...
if (!empty($_POST['fields'])) {
    foreach ($_POST['fields'] as $field_id => $fields) {
        ...
        if ($type == 'delete') {
            $database->query('DELETE FROM `'.TABLE_PREFIX.'mod_'.$mod_name.'_fields` WHERE `field_id` = '.(int)$field_id.' ');
            $database->query('DELETE FROM `'.TABLE_PREFIX.'mod_'.$mod_name.'_item_fields` WHERE `field_id` = '.(int)$field_id.' ');
        ...
        if (is_numeric($field_id)) {
            ...
            $sUpdateFields =  'UPDATE `'.TABLE_PREFIX.'mod_'.$mod_name.'_fields` SET '
                           .  ' '.$update_string.' '
                           .  'WHERE `field_id` = '.$field_id.' ';
            $database->query($sUpdateFields);
...
if ($add_fields && $new_fields > 0) {
    for ($i = 0; $i < $new_fields; $i++) {
        ...
        $Sql =  'INSERT INTO `'.TABLE_PREFIX.'mod_'.$mod_name.'_fields` SET '
                .  '`type`= \'text\', '
                .  '`extra`= \'\', '
                .  '`template`= \''.$template.'\' ';
        $database->query($Sql);
        ...
        $database->query('UPDATE `'.TABLE_PREFIX.'mod_'.$mod_name.'_fields` SET `name` = CONCAT(\''.$name.'\', '.$field_id.') WHERE `field_id` = '.(int)$field_id.' ');
        if ($database->is_error()) {
            $errors[] = $database->get_error();
            $errors[] = "Line 163";
        }
        ...
        $sSql = 'INSERT INTO `'.TABLE_PREFIX.'mod_'.$mod_name.'_item_fields` (`item_id`, `field_id`,`value`) '
